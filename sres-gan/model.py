...
class SRES_GAN_D(nn.Module):
    def __init__(self):
        ...
        self.disc_block3 = DiscBlock(64, 64, 4, 2)
        self.disc_block4 = DiscBlock(64, 128, 4, 2)
        self.disc_block5 = DiscBlock(128, 64, 4, 2)
        self.disc_block6 = DiscBlock(64, 32, 4, 2)
        self.disc_block7 = DiscBlock(32, 1, 7, 7, bn=False, act=False)
...
class SRES_GAN_G(nn.Module):
    def __init__(self):
        ...
        self.initial = ConvBlock(3, 64, k=9, stride=1, padding=4, bias=False)
        self.conv_1 = ConvBlock(64, 64, bias=False)
        self.conv_2 = ConvBlock(64, 64, bias=False)
        self.pixel_shuffle_1 = PixelShuffleBlock(64, 64)
        self.pixel_shuffle_2 = PixelShuffleBlock(64, 64)
        self.pixel_shuffle_3 = PixelShuffleBlock(64, 32)
        self.conv_out = ConvBlock(32, 3, k=9, stride=1, padding=4, bias=False, bn=False, act=False)
        self.tanh = nn.Tanh()

    def forward(self, x):
        residual = self.initial(x)
        out = self.conv_1(residual)
        out = self.conv_2(out)
        out = out + residual
